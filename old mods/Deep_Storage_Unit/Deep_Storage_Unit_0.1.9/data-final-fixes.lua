...
local fuel = drones and data.raw["recipe"]["fuel-depots"].results[1].name or nil
local categories = {
    "deep-storage-item",
    "deep-storage-fluid",
...
local create_recipe = function(item)
    ...
    if (has_flag(item, "not-stackable") or has_flag(item, "hidden") or item.name:find("ee%-") or item.name:find("electronic%-fuel")) then return end
...
local create_fluid_recipe = function(fluid)
    if ((fluid.subgroup and fluid.subgroup == "trainparts-fluid") or fluid.name == "fluid-unknown" or fluid.name:find("ee%-")) then return end
