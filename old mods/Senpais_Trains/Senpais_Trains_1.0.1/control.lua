...
script.on_init( function()
	Senpais.Functions.Script.globals()
	Senpais.Functions.Script.Players()
end )
...
script.on_event( defines.events.on_gui_click, function( event )
	...
	if name == "SenpaisGUIButton" then
		if mod_gui.get_frame_flow( player).SenpaisGUI then
			...
			Senpais.Functions.GUI.MainGUI( player )
		end
		return
	end
	if parent and parent.name == "SenpaisGUIFrame01" then
		if Senpais.GUIButtons[name] then
			Senpais.Functions.GUI.DestroyChildren( parent.parent.parent.children, 2 )
		end
		if name == "SenpaisSmarterTrainsGUIMainButton" and settings.startup["Smarter-Trains"].value then
			Senpais.Functions.GUI.SmarterTrainsGUIMain( parent.parent.parent.children[2] )
		end
		if name == "SenpaisCountTrainsButton" then
			Senpais.Functions.GUI.CountTrainsGUIMain( parent.parent.parent.children[2] )
		end
		if name == "SenpaisCountWagonsButton" then
			Senpais.Functions.GUI.CountWagonsGUIMain( parent.parent.parent.children[2] )
		end
		if name == "SenpaisCountFluidWagonsButton" then
			Senpais.Functions.GUI.CountFluidWagonsGUIMain( parent.parent.parent.children[2] )
		end
		if name == "SenpaisElectricTrainsStatesButton" then
			Senpais.Functions.GUI.ElectricTrainsStatesGUIMain( parent.parent.parent.children[2] )
		end
		return
	end
	if name == "SenpaisSmarterTrainsGUISpriteButton01" and player.opened and player.opened.train then
		if parent.parent.parent.parent.children[3].children[1] then
			...
			Sennpais.Functions.GUI.SmarterTrainsGUIAdd( parent.parent.parent.parent.children[3] )
		end
		return
	...
	if name == "SenpaisSmarterTrainsGUISpriteButton02" and parent.children[1].selected_index > 0 then
		...
		parent2.clear()
		Senpais.Functions.GUI.SmarterTrainsGUIMain( parent2 )
		return
	end
	if name == "SenpaisSmarterTrainsGUIButton" then
		if player.opened and player.opened.train and parent.children[1].children[1].text ~= nil and not global.SenpaisScheduleLines[parent.children[1].children[1].text] then
			...
			global.SenpaisScheduleLines[parent.children[1].children[1].text] = { signal = Senpais.Functions.SmarterTrains.Check( parent.children[1].children[2].elem_value ), schedule = clear_schedule }
			...
			Senpais.Functions.GUI.SmarterTrainsGUIMain( parent2.children[2] )
		else
	...
	if parent and parent.name == "SenpaisCountTrainsTable01" then
		...
		for _, z in pairs( global.PlayerDATA[player.index].TrainsCount ) do
			if element.name == "SenpaisCountTrainsButton01_" .. z.name then
				Senpais.Functions.GUI.CountTrainsGUIList( parent.parent.parent.parent.parent.parent.children[3], z )
				break
			end
		end
		return
	end
	if parent and parent.name == "SenpaisCountTrainsTable02" then
			...
			if s.name == global.PlayerDATA[player.index].EntityName then
				for v = 1, #s.entities do
					if element.name == "SenpaisCountTrainsButton02_" .. s.name .. "_" .. v then
						...
						Senpais.Functions.GUI.CountTrainsGUIEntityEdit( parent.parent.parent.parent.parent.parent.children[4], s.entities[v] )
						break
					end
				end
				break
	...
	if name == "SenpaisCountTrainsButton03" then
		for _, r in pairs( global.PlayerDATA[player.index].TrainsCount ) do
			if r.name == global.PlayerDATA[player.index].EntityName then
				...
				Senpais.Functions.GUI.DestroyChildren( parent2, 2 )
				Senpais.Functions.GUI.CountTrainsGUIMain( parent2[2] )
				break
			end
		end
		return
	...
	if parent and parent.name == "SenpaisCountWagonsTable01" then
		...
		for _, j in pairs( global.PlayerDATA[player.index].WagonsCount ) do
			if element.name == "SenpaisCountWagonsButton_" .. j.name then
				Senpais.Functions.GUI.CountWagonsGUIList( parent.parent.parent.parent.parent.parent.children[3], j )
				break
			end
		end
		return
	...
	if parent and parent.name == "SenpaisCountFluidWagonsTable01" then
		...
		for _, y in pairs( global.PlayerDATA[player.index].FluidWagonsCount ) do
			if element.name == "SenpaisCountFluidWagonsButton_" .. y.name then
				Senpais.Functions.GUI.CountFluidWagonsGUIList( parent.parent.parent.parent.parent.parent.children[3], y )
			end
		end
		return
	end
...
script.on_event( defines.events.on_gui_selection_state_changed, function( event )
	...
	if element.name == "SenpaisSmarterTrainsGUIDropDown" then
		parent.children[2].elem_value = Senpais.Functions.SmarterTrains.Check( global.SenpaisScheduleLines[global.SenpaisLines[element.selected_index]].signal )
		Senpais.Functions.GUI.SmarterTrainsGUIMainList( parent.parent )
		return
	end
	if element.name == "SenpaisCountTrainsDropDown" and settings.startup["Smarter-Trains"].value then
		Senpais.Functions.GUI.CountTrainsGUIEntityEditList( parent.children[3], element.selected_index )
	end
end )
...
script.on_event( defines.events.on_gui_elem_changed, function( event )
	...
	if name == "SenpaisSmarterTrainsGUIElemButton01" and parent.children[1].selected_index > 0 then
		global.SenpaisScheduleLines[global.SenpaisLines[parent.children[1].selected_index]].signal = Senpais.Functions.SmarterTrains.Check( element.elem_value )
		return
	end
	if parent.name == "SenpaisSmarterTrainsGUITable03" then
		for _, f in pairs( global.SenpaisScheduleLinesSignals[global.SenpaisLines[parent.parent.parent.parent.children[1].children[1].selected_index]] ) do
			if name == "SenpaisSmarterTrainsGUIElemButton_" .. f.station then
				f.signal = Senpais.Functions.SmarterTrains.Check( element.elem_value )
			end
		end
		return
	end
...
script.on_event( defines.events.on_player_created, function( event )
	if not mod_gui.get_button_flow( game.players[event.player_index] ).SenpaisGUIButton then
		local button = Senpais.Functions.GUI.Add.Sprite_Button( mod_gui.get_button_flow( game.players[event.player_index] ), "SenpaisGUIButton", "Senpais-S" )
...
script.on_event( defines.events.on_train_changed_state, function( event )
	...
	if train.state == defines.train_state.on_the_path and event.old_state == defines.train_state.wait_station and global.SenpaisTrainSchedulebyID[train.id] then
		...
		if train_global.smartertrains then
			...
			for o, p in pairs( global.SenpaisScheduleLines ) do
				if p.signal ~= nil then
					table.insert( LinieSignals, { signal = Senpais.Functions.SmarterTrains.Check( { type = p.signal.type, name = p.signal.name } ), linie = o } )
				end
			end
			...
			if LinieHighestSignal.linie ~= nil then
				...
				for _, b in pairs( global.SenpaisScheduleLinesSignals[LinieHighestSignal.linie] ) do
					if b.signal ~= nil then
						table.insert( StationSignals, { signal = Senpais.Functions.SmarterTrains.Check( { type = b.signal.type, name = b.signal.name } ), station = b.station } )
					end
				end
		...
		if train_global.coupling then
			...
			local back = Senpais.Functions.Coupling.GetRealBack( train, station )
			local schedule = train.schedule
			local changed = false
			if Senpais.Functions.Coupling.AttemptCouple( train, Senpais.Functions.Coupling.GetSignal( station, Senpais.Signals["Signal_Couple"] ), station ) then
				changed = true
				couple = true
				train = front.train
			...
			front = Senpais.Functions.Coupling.AttemptUncouple( front, Senpais.Functions.Coupling.GetSignal( station, Senpais.Signals["Signal_Uncouple"] ) )
			if front then
				changed = true
			else
				front = back
...
script.on_event( defines.events.on_tick, function( event )
	...
	if settings.startup["Senpais-Power-Provider"].value and event.tick % ( game.speed * 15 ) == 0 then
		for _, player in pairs( game.players ) do
			...
			if GUI and GUI.SenpaisGUITable01.SenpaisGUIHiddenFrame02.children[1] ~= nil and GUI.SenpaisGUITable01.SenpaisGUIHiddenFrame02.children[1].name == "SenpaisElectricTrainsStatesFrame01" then
				mod_gui.get_frame_flow( player).SenpaisGUI.SenpaisGUITable01.SenpaisGUIHiddenFrame02.clear()
				Senpais.Functions.GUI.ElectricTrainsStatesGUIMain( mod_gui.get_frame_flow( player).SenpaisGUI.SenpaisGUITable01.SenpaisGUIHiddenFrame02 )
			end
		end
	end
end )

script.on_event( defines.events.on_built_entity, Senpais.Functions.Script.OnBuild )
script.on_event( defines.events.on_robot_built_entity, Senpais.Functions.Script.OnBuild )
script.on_event( defines.events.on_pre_player_mined_item, Senpais.Functions.Script.OnRemove )
script.on_event( defines.events.on_robot_pre_mined, Senpais.Functions.Script.OnRemove )
script.on_event( defines.events.on_entity_died, Senpais.Functions.Script.OnRemove )
