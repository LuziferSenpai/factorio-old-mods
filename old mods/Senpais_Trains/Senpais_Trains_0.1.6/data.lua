...
local function replace_filenames_in_layer( layer )
	if layer.filenames then
		for i, filename in pairs( layer.filenames ) do
			layer.filenames[i] = string.gsub( filename, "__base__/graphics/entity/diesel-locomotive", MODNAME .. "/graphics" )
		end
	end
	if layer.filename then
		layer.file = string.gsub( layer.filename, "__base__/graphics/entity/diesel-locomotive", MODNAME .. "/graphics" )
	end
end
...
function Senpais.Functions.Create.Elec_Locomotive( multiplier, name, icon, health, weight, speed, color, grid, subgroup, order, stack, ingredients, tech )
	...
	replace_mask_image_filenames( train_entity.pictures.layers )
...
function Senpais.Functions.Create.Battle_Wagon( name, icon, inventory, health, weight, speed, color, grid, subgroup, order, stack, ingredients, tech )
	...
	replace_mask_image_filenames( wagon_entity.pictures.layers )
	replace_mask_image_filenames( wagon_entity.horizontal_doors.layers )
	replace_mask_image_filenames( wagon_entity.vertical_doors.layers )
...
if settings["startup"]["Battle-Wagon-1"].value == true then
	Senpais.Functions.Create.Battle_Wagon( "Battle-Wagon-1", "__base__/graphics/icons/cargo-wagon.png", 20, 1000, 2000, 1.2, { r = 19, g = 122, b = 156 }, "Senpais-Trains", "transport",
										   "a[train-system]-gaa[Battle-Wagon-1]", 5, { { "locomotive", 1 }, { "modular-armor", 1 } }, "modular-armor" )
