...
local function replace_filenames_in_layer( layer, entity )
...
local function replace_mask_image_filenames( layers, entity )
	for _, layer in pairs( layers ) do
		if layer.apply_runtime_tint == true then
			replace_filenames_in_layer( layer, entity )
			if layer.hr_version then
				replace_filenames_in_layer( layer.hr_version, entity )
			end
		end
	end
end
...
function Senpais.Functions.Create.Elec_Locomotive( multiplier, name, icon, health, weight, speed, color, grid, subgroup, order, stack, ingredients, tech )
	...
	replace_mask_image_filenames( train_entity.pictures.layers, "diesel-locomotive" )
...
function Senpais.Functions.Create.Battle_Wagon( name, icon, inventory, health, weight, speed, color, grid, subgroup, order, stack, ingredients, tech )
	...
	replace_mask_image_filenames( wagon_entity.pictures.layers, "cargo-wagon" )
	replace_mask_image_filenames( wagon_entity.horizontal_doors.layers, "cargo-wagon" )
	replace_mask_image_filenames( wagon_entity.vertical_doors.layers, "cargo-wagon" )
...
--[[if settings["startup"]["Battle-Laser"].value == true then
	local laser_beam =
	{
	 ...
	 ending = { filename = MODNAME .. "/graphics/laser-beam-end.png", line_length = 16, tint = { r = 232, g = 10, b = 165 }, frame_count = 12, x = 45 * 4, width = 45, height = 1, priority = "high", animation_speed = 0.5, blend_mode = "additive-soft" },
...
if settings["startup"]["Senpais-Dora"].value == true then
	...
	dora_projectile.action.action_delivery.target_effects[1] = { type = "nested-result", action = { type = "area", target_entities = false, repeat_count = 1000,
																 radius = 20, action_delivery = { type = "projectile", projectile = "atomic-bomb-wave",
																 starting_speed = 0.5 } } }
	table.insert( dora_projectile.action.action_delivery.target_effects, { repeat_count = 250, type = "create-trivial-smoke", smoke_name = "nuclear-smoke",
																	offset_deviation = { { -1, -1}, { 1, 1 } }, slow_down_factor = 1, starting_frame = 3,
																	starting_frame_deviation = 5, starting_frame_speed = 0, starting_frame_speed_deviation = 5,
																	speed_from_center = 0.5, speed_deviation = 0.2 } )
	table.insert( dora_projectile.action.action_delivery.target_effects, { type = "damage", damage = { amount = 1000, type = "explosion" } } )
	table.insert( dora_projectile.action.action_delivery.target_effects, { type = "create-entity", entity_name = "small-scorchmark", check_buildability = true } )
	table.insert( dora_projectile.action.action_delivery.target_effects, { type = "destroy-cliffs", radius = 10, explosion = "explosion" } )
	...
	dora_ammo_recipe.ingredients = { { "artillery-shell", 1 }, { "atomic-bomb", 1 }, { "explosives", 20 } }
	...
	data:extend( { { type = "ammo-category", name = "dora-shell" }, dora_entity, dora_gun, dora_ammo, dora_projectile, dora_item, dora_recipe, dora_ammo_recipe, dora_tech } )
end