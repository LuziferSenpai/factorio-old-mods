...
for index, force in pairs(game.forces) do
  ...
  if tech["armor-making-3"].researched then
    recipes["battle-cargo-1"].enabled = true
    recipes["battle-loco-1"].enabled = true
    if recipes["Senpais-Electric-Train"] then
      recipes["elec-battle-loco-1"].enabled = true
    end
  end
